...
export default function MembershipPage() {
  ...
  const selectedStats = useMemo(() => {
    ...
    const customerOrders = orders.filter((order) => order.pelangganId === edit.pelangganId);
    const completedOrders = customerOrders.filter((order) => order.status === "COMPLETED");
    const paidOrders = completedOrders.filter((order) => order.statusPembayaran === "PAID");
    const lastVisit = [...completedOrders].sort(
      (a, b) => new Date(b.diperbaruiPada).getTime() - new Date(a.diperbaruiPada).getTime(),
    )[0];
  ...
  const save = useMutation({
    mutationFn: async () => {
      if (edit) {
        return (await api.patch<{ data: Membership }>(`/memberships/${edit.id}`, {
          discountPercent: discount,
          isActive: active,
        })).data.data;
      } else {
        return (await api.post<{ data: Membership }>("/memberships", {
    ...
    onSuccess: async (savedMembership) => {
      ...
      toast.success(
        edit
          ...
          : `Membership ${savedMembership.memberCode} created automatically`,
      );
      setOpen(false);
    },
  ...
  return (
    <>
      <PageTitle
        ...
        action={canManage ? <Button onClick={() => start()}>+ New Membership</Button> : undefined}
      />
      ...
      <Card className="overflow-hidden">
        ...
        <div className="overflow-x-auto">
          {isLoading ? (
            ...
            <ErrorState text="Failed to load memberships." onRetry={() => void refetch()} />
          ) : !filteredMemberships.length ? (
            <Empty text={search || statusFilter !== "ALL" ? "No memberships match your filters" : "No memberships yet"} />
          ) : (
              ...
              <tbody>
                {filteredMemberships.map((membership) => (
                  ...
                  >
                    ...
                    <td>
                      <StatusBadge value={membership.isActive ? "AKTIF" : "DIBLOKIR"} />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <Modal
        open={open}
        title={edit ? (canManage ? "Edit Membership" : "Membership Details") : "New Membership"}
      ...
      >
        <div className="space-y-4">
          {edit && selectedStats && (
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <p className="mt-1 font-bold text-slate-900">{tanggal(edit.joinedAt)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Total Visits</p>
                <p className="mt-1 font-bold text-slate-900">{selectedStats.totalVisits}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Total Spending</p>
                <p className="mt-1 font-bold text-slate-900">{rupiah(selectedStats.totalSpending)}</p>
              </div>
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                <p className="text-[11px] font-semibold uppercase tracking-wide text-slate-400">Last Visit</p>
                <p className="mt-1 font-bold text-slate-900">
                  {selectedStats.lastVisit ? tanggal(selectedStats.lastVisit) : "No visit yet"}
                </p>
              </div>
            </div>
          )}
          ...
          {!edit && (
            <div>
              ...
              <SearchableSelect
                ...
                disabled={!canManage}
                options={customers
                  .filter((item) => item.status === "AKTIF" && !item.membership)
                  .map((item) => ({
                    value: item.id,
          ...
          {edit && (
            <div>
              <Label>Customer</Label>
              <Input value={`${edit.pelanggan?.nama ?? "-"} — ${edit.pelanggan?.nomorTelepon ?? "-"}`} disabled />
            </div>
          )}
          ...
          {canManage && (
            <Button
              className="w-full"
              onClick={() => save.mutate()}
              disabled={(!edit && !customer) || save.isPending}
            >
